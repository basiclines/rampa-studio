...
import { palette } from '../../sdk/src/palette';
...
interface PaletteArgs {
  filePath: string;
  mode: 'dominant' | 'raw' | 'ansi' | 'average' | 'temperature';
  count: number;
  tolerance: number;
  rawTolerance: number;
  maxColors: number;
...
export async function runPalette(args: string[]): Promise<void> {
  ...
  try {
    ...
    switch (parsed.mode) {
      case 'dominant': {
        const dom = p.dominant({ count: parsed.count, tolerance: parsed.tolerance });
      ...
      case 'raw': {
        const raw = p.raw({ tolerance: parsed.rawTolerance, maxColors: parsed.maxColors });
        if (parsed.output === 'json') {
      ...
      case 'ansi': {
        ...
        for (const name of names) {
          ...
          if (entries.length === 0) {
            ...
            const swatches = entries.map((e: any) => `${swatch(e.color.hex)} ${e.color.hex}`).join('  ');
