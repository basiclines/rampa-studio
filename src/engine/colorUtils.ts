...
const calculateScalePosition = (i: number, steps: number, scaleType: string): number => {
  try {
...
const calculateGeometricPosition = (i: number, steps: number): number => {
  try {
    const ratio = 3;
    if (steps <= 1) return 0;
    const min = 1;
...
const calculateFibonacciPositions = (steps: number): number[] => {
  try {
    const fibs = [0, 1];
...
const calculateGoldenRatioPositions = (steps: number): number[] => {
  try {
...
const calculateLogarithmicPosition = (i: number, steps: number): number => {
  try {
    const min = 1;
    const max = steps;
...
const calculatePowersOf2Position = (i: number, steps: number): number => {
  try {
    const min = 1;
...
const calculateMusicalRatioPosition = (i: number, steps: number): number => {
  try {
...
const calculateCielabUniformPosition = (i: number, steps: number): number => {
  try {
...
const calculateEaseInPosition = (i: number, steps: number): number => {
  try {
    const t = i / (steps - 1);
    const result = t * t;
    return clampValue(result, 0, 1);
...
const calculateEaseOutPosition = (i: number, steps: number): number => {
  try {
    const t = i / (steps - 1);
...
const calculateEaseInOutPosition = (i: number, steps: number): number => {
  try {
    const t = i / (steps - 1);
...
const calculatePositions = (i: number, config: ColorRampConfig) => {
